...
def get_data_feature(mogo_url, database, collection_name):
    ...
    data_frature_frame = pd.DataFrame(data_feature, columns=column)
    return data_frature_frame
...
def data_process(data, iot_dict):
    ...
    for i in range(length):
        print('正在处理第{}条流，一共{}条'.format(i, length))
        quintuple = flow[i]
        ...
        if ip1 in iot_dict or (re.match(ip_10_0, ip1) is not None):
            ip = ip1
        elif ip2 in iot_dict or (re.match(ip_10_0, ip2) is not None):
            ip  = ip2
        else:
            ip  = ip1
        '''
...
def pred_test_data(model, data_frame, iot_dict, threshold):
    ...
    in_iot_dict = isin_iot_dict[isin_iot_dict != -1]
    ...
    learn_data_index = in_iot_dict.map(lambda x: x not in model_class_lst)
    learn_data_notin_dict = isin_iot_dict_data[learn_data_index]

    isin_iot_dict_in_model_index = in_iot_dict.map(lambda x: x in model_class_lst)
    isin_iot_dict_in_model_data = isin_iot_dict_data[isin_iot_dict_in_model_index]
    isin_iot_dict_in_model_data.reset_index(drop=True, inplace=True)

    data_test = isin_iot_dict_in_model_data.iloc[:, :-2]
    ...
    smaller_threshold_data = isin_iot_dict_in_model_data[smaller_threshold_idx]
    ...
    greater_threshold_data = isin_iot_dict_in_model_data[greater_threshold_idx]
    ...
    '''
    learn_data = pd.concat([learn_data, learn_data_notin_dict])
...
def pred_topred_data(model, pred_data, threshold=0.8):
    ...
    :return:每个iP的预测类别
    '''
    pred_prob = pd.DataFrame(model.predict_proba(pred_data.iloc[:, :-2]), columns=model.classes_, index=pred_data['ip'])
...
if __name__ == "__main__":
    ...
    # data_feature_frame = get_data_feature('172.18.130.22:26000', 'ion', 'sessionFeatureTest0' )
